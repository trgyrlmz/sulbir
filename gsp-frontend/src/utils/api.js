...
export const API_ENDPOINTS = {
    ...
    SULAMA: {
        ...
        BARAJ_DEPOLAMA_DEGISIM: `${API_BASE_URL}/api/sulama/dashboard/baraj_depolama_degisim/`,
        AYLIK_BRUT_SU_IHTIYACI: `${API_BASE_URL}/api/sulama/dashboard/aylik_brut_su_ihtiyaci/`,
    }
};
...
export const sulamaAPI = {
    ...
    hesaplaKanalSuHacmi: (kanalId, yukseklik) => apiRequest(
        `${API_ENDPOINTS.SULAMA.GUNLUK_SEBEKE_SU}hesapla_kanal_su_hacmi/`,
        {
            method: 'POST',
            body: JSON.stringify({ kanal: kanalId, yukseklik })
        }
    ),
    ...
    // Baraj Depolama Değişim Verileri
    getBarajDepolamaDegisim: (params = {}) => {
        const searchParams = new URLSearchParams(params);
        const url = searchParams.toString() 
            ? `${API_ENDPOINTS.SULAMA.BARAJ_DEPOLAMA_DEGISIM}?${searchParams.toString()}`
            : API_ENDPOINTS.SULAMA.BARAJ_DEPOLAMA_DEGISIM;
        return apiRequest(url);
    },

    // Aylık Brüt Su İhtiyaçları
    getAylikBrutSuIhtiyaci: (params = {}) => {
        const searchParams = new URLSearchParams(params);
        const url = searchParams.toString() 
            ? `${API_ENDPOINTS.SULAMA.AYLIK_BRUT_SU_IHTIYACI}?${searchParams.toString()}`
            : API_ENDPOINTS.SULAMA.AYLIK_BRUT_SU_IHTIYACI;
        return apiRequest(url);
    },
};
