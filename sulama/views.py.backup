...
class GunlukSebekeyeAlinanSuMiktariViewSet(SulamaBazliMixin, viewsets.ModelViewSet):
    ...
    def hesapla_su_miktari(self, request):
        """Yükseklik değerine göre su miktarını hesapla - Gelişmiş interpolasyon ile"""
        ...
        try:
            ...
            # Kanal abağından su miktarını hesapla - interpolasyon ile
            try:
                from .models import KanalAbak
                ...
                try:
                    ...
                    return Response({
                        ...
                        'yukseklik': yukseklik,
                        'kanal': kanal.isim,
                        'hesaplama_tipi': 'tam_eslesme',
                        'success': True
                    })
                except KanalAbak.DoesNotExist:
                    # Tam eşleşme yoksa interpolasyon yap
                    abaklar = kanal.abaklar.all().order_by('yukseklik')
                    
                    if not abaklar.exists():
                        return Response({
                            'error': 'Bu kanal için abak verisi bulunamadı',
                            'su_miktari': 0,
                            'success': False
                        }, status=status.HTTP_400_BAD_REQUEST)
                    
                    # Girilen yükseklik en küçük değerden küçükse
                    if yukseklik <= abaklar.first().yukseklik:
                        return Response({
                            'su_miktari': abaklar.first().hacim,
                            'yukseklik': yukseklik,
                            'kanal': kanal.isim,
                            'hesaplama_tipi': 'min_deger',
                            'success': True
                        })
                    
                    # Girilen yükseklik en büyük değerden büyükse
                    if yukseklik >= abaklar.last().yukseklik:
                        return Response({
                            'su_miktari': abaklar.last().hacim,
                            'yukseklik': yukseklik,
                            'kanal': kanal.isim,
                            'hesaplama_tipi': 'max_deger',
                            'success': True
                        })
                    
                    # İki değer arasında interpolasyon yap
                    for i in range(len(abaklar) - 1):
                        alt_abak = abaklar[i]
                        ust_abak = abaklar[i + 1]
                        
                        if alt_abak.yukseklik <= yukseklik <= ust_abak.yukseklik:
                            # Linear interpolasyon
                            x1, y1 = alt_abak.yukseklik, alt_abak.hacim
                            x2, y2 = ust_abak.yukseklik, ust_abak.hacim
                            
                            # y = y1 + (y2-y1) * (x-x1) / (x2-x1)
                            interpolated_hacim = y1 + (y2 - y1) * (yukseklik - x1) / (x2 - x1)
                            
                            return Response({
                                'su_miktari': round(interpolated_hacim, 6),
                                'yukseklik': yukseklik,
                                'kanal': kanal.isim,
                                'hesaplama_tipi': 'interpolasyon',
                                'alt_abak': {'yukseklik': x1, 'hacim': y1},
                                'ust_abak': {'yukseklik': x2, 'hacim': y2},
                                'success': True
                            })
                    
                    return Response({
                        'error': f'Bu kanal için {yukseklik} m yükseklik değeri hesaplanamadı',
                        'su_miktari': 0,
                        'success': False
                    }, status=status.HTTP_400_BAD_REQUEST)
                ...
                return Response({
                    'error': f'Hesaplama sırasında hata oluştu: {str(e)}',
                    'su_miktari': 0,
                    'success': False
    ...
    @action(detail=False, methods=['post'])
    def hesapla_kanal_su_hacmi(self, request):
        """Kanal abağından su hacmi hesapla - Yeni gelişmiş endpoint"""
        kanal_id = request.data.get('kanal')
        yukseklik = request.data.get('yukseklik')
        ...
        try:
            ...
            # Kanal abağından su hacmini hesapla
            try:
                from .models import KanalAbak
                ...
                try:
                    ...
                    for i in range(len(abaklar) - 1):
                        ...
                        if alt_abak.yukseklik <= yukseklik <= ust_abak.yukseklik:
                            ...
                            return Response({
                                'su_hacmi': round(interpolated_hacim, 6),
                                'yukseklik': yukseklik,
                                'kanal': kanal.isim,
                                'hesaplama_tipi': 'interpolasyon',
                                'alt_abak': {'yukseklik': x1, 'hacim': y1},
...
class DashboardViewSet(SulamaBazliMixin, viewsets.ViewSet):
    ...
    def aylik_su_kullanimi(self, request):
        ...
        try:
            ...
            # Toplam depolama kapasitesi: Depolama tesislerinin maksimum_hacim alanından
            toplam_depolama_kapasitesi = 0
            if depolama_tesisi_id:
                # Tek tesis seçiliyse, o tesisin maksimum kapasitesi
                try:
                    tesis = DepolamaTesisi.objects.get(id=depolama_tesisi_id)
                    # DepolamaTesisi modelindeki maksimum_hacim alanını kullan
                    if tesis.maksimum_hacim:
                        toplam_depolama_kapasitesi = float(tesis.maksimum_hacim)
                except:
                    pass
            else:
                # Tüm yetkili tesisler için maksimum kapasiteleri topla
                # Yetki kontrolü ile tüm tesisleri al, sadece veri girişi yapılanları değil
                yetkili_tesisler = self.filter_by_sulama_permission(
                    DepolamaTesisi.objects.all(), 'sulama'
                ).distinct()
            ...
            toplam_istatistikler = {
                ...
                'toplam_depolama_su': toplam_depolama_kapasitesi,  # Maksimum kapasiteler
    ...
    @action(detail=False, methods=['get'])
    def baraj_depolama_degisim(self, request):
        """
        Baraj depolama değişim tablosu
        Aylık depolama durumları ve aylar arası değişimler
        """
        try:
            # Parametreler
            yil = request.GET.get('yil', datetime.now().year)
            ...
            # Depolama verilerini al
            depolama_qs = GunlukDepolamaTesisiSuMiktari.objects.filter(tarih__year=yil)
            
            # Depolama tesisi filtreleme
            if depolama_tesisi_id:
                try:
                    depolama_tesisi_id = int(depolama_tesisi_id)
                    depolama_qs = depolama_qs.filter(depolama_tesisi_id=depolama_tesisi_id)
            ...
            baraj_degisim_verileri = []
            onceki_ay_depolama = 0

            for ay_idx, (ay_no, ay_isim) in enumerate(aylar):
                ay_num = int(ay_no)
                
                # Bu ay için depolama tesislerinin son kayıtlarını al
                aylik_depolama_toplam = 0
                aylik_depolama_kayit = 0
                ...
                # Net değişim hesapla (önceki aydan fark)
                net_degisim = 0
                if ay_idx > 0:  # İlk ay değilse
                    net_degisim = aylik_depolama_toplam - onceki_ay_depolama
                
                # Veri ekle
                baraj_degisim_verileri.append({
                    'ay': ay_isim,
                    'ay_no': ay_num,
                    'depolama_miktari': float(aylik_depolama_toplam),
                    'net_degisim': float(net_degisim),
                    'degisim_tipi': 'giris' if net_degisim > 0 else 'cikis' if net_degisim < 0 else 'sabit',
                    'kayit_sayisi': aylik_depolama_kayit
                })
                
                # Sonraki ay için güncelle
                onceki_ay_depolama = aylik_depolama_toplam

            # Toplam istatistikler
            toplam_istatistikler = {
                'yil': yil,
                'depolama_tesisi_id': depolama_tesisi_id,
                'toplam_depolama_degisim': sum(veri['net_degisim'] for veri in baraj_degisim_verileri),
                'toplam_giris': sum(veri['net_degisim'] for veri in baraj_degisim_verileri if veri['net_degisim'] > 0),
                'toplam_cikis': abs(sum(veri['net_degisim'] for veri in baraj_degisim_verileri if veri['net_degisim'] < 0)),
                'baslangic_depolama': baraj_degisim_verileri[0]['depolama_miktari'] if baraj_degisim_verileri else 0,
                'bitis_depolama': baraj_degisim_verileri[-1]['depolama_miktari'] if baraj_degisim_verileri else 0
            }

            return Response({
                'aylik_veriler': baraj_degisim_verileri,
                'istatistikler': toplam_istatistikler,
                'success': True
            })
            ...
            return Response(
                {'error': f'Baraj depolama değişim verileri alınırken hata oluştu: {str(e)}'}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

    @action(detail=False, methods=['get'])
    def aylik_brut_su_ihtiyaci(self, request):
        """
        Aylık brüt su ihtiyaçları
        Sulama hesaplama sayfasındaki hesaplamalara göre aylık brüt su ihtiyaçları
        """
        try:
            # Parametreler
            yil = request.GET.get('yil', datetime.now().year)
            ...
            # Base queryset - YillikUrunDetay'dan veri al
            urun_detay_qs = YillikUrunDetay.objects.filter(yillik_tuketim__yil=yil).select_related('urun', 'yillik_tuketim__sulama')
            ...
            if depolama_tesisi_id:
                try:
                    ...
                    try:
                        depolama_tesisi = DepolamaTesisi.objects.get(id=depolama_tesisi_id)
                        urun_detay_qs = urun_detay_qs.filter(yillik_tuketim__sulama_id=depolama_tesisi.sulama_id)
                    except DepolamaTesisi.DoesNotExist:
                        pass
                except (ValueError, TypeError):
                    pass
            # Eğer depolama tesisi seçilmemişse sulama filtreleme
                ...
                try:
                    sulama_id = int(sulama_id)
                    urun_detay_qs = urun_detay_qs.filter(yillik_tuketim__sulama_id=sulama_id)
                except (ValueError, TypeError):
                    pass

            # Yetki kontrolü
            urun_detay_qs = self.filter_by_sulama_permission(urun_detay_qs, 'yillik_tuketim__sulama')
            ...
            aylik_brut_ihtiyac = {}
            for ay_no, ay_isim in aylar:
                aylik_brut_ihtiyac[ay_no] = {
                    'ay': ay_isim,
                    'ay_no': int(ay_no),
                    'brut_su_ihtiyaci': 0,  # hm³ cinsinden
                    'toplam_alan': 0,  # ha cinsinden
                    'urun_sayisi': 0
                }

            # YillikUrunDetay verilerini işle
            for urun_detay in urun_detay_qs:
                # Bu ürün için aylık UR değerlerini al
                ur_values = [
                    float(urun_detay.urun.ocak or 0),
                    float(urun_detay.urun.subat or 0),
                    float(urun_detay.urun.mart or 0),
                    float(urun_detay.urun.nisan or 0),
                    float(urun_detay.urun.mayis or 0),
                    float(urun_detay.urun.haziran or 0),
                    float(urun_detay.urun.temmuz or 0),
                    float(urun_detay.urun.agustos or 0),
                    float(urun_detay.urun.eylul or 0),
                    float(urun_detay.urun.ekim or 0),
                    float(urun_detay.urun.kasim or 0),
                    float(urun_detay.urun.aralik or 0)
                ]

                alan = float(urun_detay.alan or 0)
                ciftlik_randi = float(urun_detay.yillik_tuketim.ciftlik_randi or 80)
                iletim_randi = float(urun_detay.yillik_tuketim.iletim_randi or 85)

                # Her ay için brüt su ihtiyacını hesapla
                for ay_idx, ur_value in enumerate(ur_values):
                    if ur_value and alan > 0:
                        # Net su ihtiyacı: Alan × UR ÷ 100000 (hm³)
                        net_su = (alan * float(ur_value)) / 100000
                        
                        # Çiftlik su ihtiyacı: Net su × 100 ÷ çiftlik_randımanı
                        ciftlik_su = (net_su * 100) / ciftlik_randi
                        
                        # Brüt su ihtiyacı: Çiftlik su × 100 ÷ iletim_randımanı
                        brut_su = (ciftlik_su * 100) / iletim_randi
                        
                        ay_no = f"{ay_idx + 1:02d}"
                        if ay_no in aylik_brut_ihtiyac:
                            aylik_brut_ihtiyac[ay_no]['brut_su_ihtiyaci'] += brut_su
                            aylik_brut_ihtiyac[ay_no]['toplam_alan'] += alan
                            aylik_brut_ihtiyac[ay_no]['urun_sayisi'] += 1

            # Sonuçları listeye çevir
            aylik_veriler = []
            for ay_no, ay_isim in aylar:
                veri = aylik_brut_ihtiyac[ay_no]
                aylik_veriler.append({
                    'ay': ay_isim,
                    'ay_no': int(ay_no),
                    'brut_su_ihtiyaci': round(veri['brut_su_ihtiyaci'], 3),
                    'toplam_alan': round(veri['toplam_alan'], 2),
                    'urun_sayisi': veri['urun_sayisi']
                })
            ...
            toplam_istatistikler = {
                ...
                'sulama_id': sulama_id,
                'depolama_tesisi_id': depolama_tesisi_id,
                'toplam_brut_ihtiyac': round(sum(veri['brut_su_ihtiyaci'] for veri in aylik_veriler), 3),
                'toplam_alan': round(sum(veri['toplam_alan'] for veri in aylik_veriler), 2),
                'toplam_urun_sayisi': sum(veri['urun_sayisi'] for veri in aylik_veriler)
            }
            ...
            return Response(
                {'error': f'Aylık brüt su ihtiyaçları alınırken hata oluştu: {str(e)}'}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

    @action(detail=False, methods=['get'])
    def mevcut_yillar(self, request):
        """
        Dashboard'da veri olan yılları getir
        """
        try:
            # Şebeke su verilerinde hangi yıllar var
            sebeke_yillar = GunlukSebekeyeAlinanSuMiktari.objects.values_list(
                'tarih__year', flat=True
            ).distinct().order_by('tarih__year')
            
            # Depolama su verilerinde hangi yıllar var
            depolama_yillar = GunlukDepolamaTesisiSuMiktari.objects.values_list(
                'tarih__year', flat=True
            ).distinct().order_by('tarih__year')
            
            # Tüketim verilerinde hangi yıllar var
            tuketim_yillar = YillikGenelSuTuketimi.objects.values_list(
                'yil', flat=True
            ).distinct().order_by('yil')
            
            # Tüm yılları birleştir ve sırala
            tum_yillar = sorted(set(list(sebeke_yillar) + list(depolama_yillar) + list(tuketim_yillar)))
            ...
            return Response(
                {'error': f'Mevcut yıllar alınırken hata oluştu: {str(e)}'}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
